...
<script>
    ...
    let routeTherapistTime = "{{ route('getTherapistTime') }}";
    let postcode = "{{ session('booking.postcode') }}";
    let fullpostcode = "{{ session('booking.input_postal_code') }}";

    // data
    @if(session('booking.asap') === 'yes')
    setTimeout(() => {
        $('.massage_me_now').trigger('click');
    }, 500);
    @endif
...
<script>
    ...
    $('.massage_me_now').click(function() {
        ...
        $.post(routeTime, {
            ...
            if (response) {
                ...
                initOwlOnTime();
                $('.calendar_time').removeClass('active');
                $('#first_time').addClass('active');
                $('#time').val($('#first_time').data('time'));
                $('#asap').val('yes');
    ...
    $("#flip_over").click(function() {
        var $this = $('#front_body');
        if ($this.hasClass("hide-elem")) {
            $('#front_body').removeClass("hide-elem").addClass("show-elem");
            $('#back_body').removeClass("show-elem").addClass("hide-elem");
            $('#map_back_body').removeClass("show-elem").addClass("hide-elem");
            $('#text_img_body').empty();
            $('#text_img_body').text('Front of body');
        } else {
            $('#front_body').removeClass("show-elem").addClass("hide-elem");
            $('#back_body').removeClass("hide-elem").addClass("show-elem");
            $('#map_back_body').removeClass("hide-elem").addClass("show-elem");
            $('#text_img_body').empty();
            $('#text_img_body').text('Back of body');
        }
    });
</script>
